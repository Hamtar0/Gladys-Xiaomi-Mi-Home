    ...
    */
    ...
    return gladys.device.create(newDevice)
        .then(async ({ types: [_status, _rotation, _battery] }) => {
            if (status) gladys.deviceState.create({ devicetype: _status.id, value: CUBE_TYPE_MAPPING[status] })
            if (rotation) gladys.deviceState.create({ devicetype: _rotation.id, value: rotation })
